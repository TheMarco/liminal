...
> landmarks: LAST CHANCE (eight cabinets: one fully active wheel, seven dim standby units), a warm
...
behind them. Its lights may die, or the hostile director may queue a real
clearance-checked encounter. Hostile figures are never installed as motionless
room geometry and never wait on an invisible room boundary.
...
- the saved VHS EFFECT and CRT EFFECT choices independently control the two
  presentation stages; no gameplay key changes either one;
...
| `scripts/shadow_figure.gd` | Burnable hostile figures and topology-aware multi-door chase. |
...
| `scripts/post_process_controller.gd` | `PostProcessController` | Independent VHS/CRT presentation state, glitches, corruption and damage presentation. |
...
start on a freshly built casino. The Wander button still starts immediately
...
- Wander button: emit `mode_selected(false)`, then `started(false)` and fade away
  exactly as the title does now.
- Descent button: emit `mode_selected(true)`, replace the menu with the rule
  card, and wait.
- Descend / Continue button: enable once preparation finishes, then emit
  `started(true)` and fade the rule card away when activated.
- Buttons use plain action labels, without action-specific keyboard shortcuts.
  Tab/arrows select a button; Enter/Space activate only the focused button.
  Escape returns informational pages to the title. The prologue similarly uses
  plain Pause and Skip Intro buttons (Skip only after a completed first viewing).
...
- `V` and `B` do nothing and are omitted from the Descent controls;
- VHS and CRT both default on, restoring the original combined presentation;
  either may be changed independently in Settings;
- the existing `--nocrt` command-line flag remains a visual-debug override
  that disables both stages;
...
The summary shows elapsed time without a rule-break count. Loss guidance is
optional; figure catches show the message above, while blackout catches explain
that movement gave the player away. Successful runs show no loss guidance.
...
2. **the hunter** — queue one immediately active hostile through the ordinary
   floor, body-clearance and line-of-sight validated encounter spawner.
...
- streamed rooms reject hostile encounter requests without changing fixtures;
- encounter figures immediately approach, burn and kill like timer-spawned encounters;
