...
func mall_fixture_audit() -> Dictionary:
	...
	for node in chunk.find_children("*", "MeshInstance3D", true, false):
		...
		var want = 0.0
		for entry in chunk.MALL_SIGN_FACES:
			if entry[0] == name:
				want = entry[1]
				break
		if want <= 0.0 or fit.y <= 0.0 \
				or absf(fit.x / fit.y - want) > 0.02 \
				or fit.x > chunk.MALL_SIGN_MAX_W + 0.001 \
				or fit.y > chunk.MALL_SIGN_MAX_H + 0.001:
			report["violations"] += 1
	for node in chunk.find_children("*", "Node3D", true, false):
		...
		if node.has_meta("mall_exit_mount") and node is MeshInstance3D:
			...
			if int(mi.get_meta("hanger_count", 0)) != 2 \
					...
					or sign_top >= chunk.ceil_h:
				report["violations"] += 1
	if chunk.style == WorldGen.MALL_FOODCOURT:
...
func wall_art_audit() -> Dictionary:
	...
	var split = chunk._resolved_room_split()
	for node in chunk.find_children("*", "Node3D", true, false):
		...
		if path.is_empty() or absf(shown - source) > 0.002 \
				or dir < 0 or dir > 3 \
				or not bool(WorldGen.edge_info(chunk.wseed, chunk.cell, dir, chunk.theme)["wall"]) \
				or size.x <= 0.0 or size.y <= 0.0 \
				or p.y - size.y * 0.5 < 0.25 \
				or p.y + size.y * 0.5 > chunk.ceil_h - 0.12 \
				or along - size.x * 0.5 < 0.20 \
				or along + size.x * 0.5 > chunk.S - 0.20 \
