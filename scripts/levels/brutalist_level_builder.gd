...
func _brutalist_floor_ceiling() -> void:
	chunk._box(Vector3(chunk.S * 0.5, -0.18, chunk.S * 0.5),
		Vector3(chunk.S, 0.36, chunk.S), Mats.brutal_floor())
	chunk._box(Vector3(chunk.S * 0.5, chunk.ceil_h + 0.24, chunk.S * 0.5),
		Vector3(chunk.S, 0.48, chunk.S), Mats.brutal_structure())
	# Deep coffers turn every ceiling into load-bearing mass, not a flat lid.
	var axis_x := WorldGen.r01(chunk.wseed, chunk.room_root.x,
		chunk.room_root.y, 2100) < 0.5
	for t in [1.4, 4.45, 7.55, 10.6]:
		var p := Vector3(6.0, chunk.ceil_h - 0.32, t) if axis_x \
			else Vector3(t, chunk.ceil_h - 0.32, 6.0)
		var s := Vector3(12.0, 0.62, 0.48) if axis_x \
			else Vector3(0.48, 0.62, 12.0)
		chunk._box(p, s, Mats.brutal_structure(), false)
	# A few rooms open to a cold, unreachable light well.
	if chunk.style == WorldGen.BRUTAL_ATRIUM \
			or chunk.style == WorldGen.BRUTAL_SANCTUM:
		chunk._box(Vector3(6.0, chunk.ceil_h - 0.018, 6.0),
			Vector3(4.6, 0.025, 3.0), Mats.brutal_panel(), false)
		for x in [3.62, 8.38]:
			chunk._box(Vector3(x, chunk.ceil_h - 0.08, 6.0),
...
func _monolith_incursion() -> void:
	if chunk.cell == Vector2i.ZERO:
		return
	var chance := 0.075 if chunk.style == WorldGen.BRUTAL_PASSAGE else 0.125
	if chunk._r(2230) >= chance:
		return
	var root := Node3D.new()
	...
	_monolith_tendril(root, centre + Vector3(-sx * 0.10, 0.0, -sz * 0.08),
		centre + Vector3(sx * 0.12, -0.66 - chunk._r(2233) * 0.34,
			-sz * 0.25), 0.032)
	...
	if chunk._r(2234) < 0.34:
		var pivot := Node3D.new()
		pivot.name = "MonolithVineKnot"
		pivot.position = centre + Vector3(-sx * 0.16, -0.08, -sz * 0.12)
		pivot.rotation = Vector3(0.0, chunk._r(2235) * TAU, PI * 0.5)
		pivot.scale = Vector3.ONE * (0.16 + chunk._r(2236) * 0.05)
...
func _brutalist_lighting() -> void:
	var dead := chunk.cell != Vector2i.ZERO and chunk._r(2120) < 0.055
	var flicker := not dead and chunk.cell != Vector2i.ZERO and chunk._r(2121) < 0.13
	var lens: StandardMaterial3D = Mats.panel_dead() if dead else Mats.brutal_panel()
	if flicker:
		lens = Mats.brutal_panel().duplicate()
	var corridor_axis := WorldGen.corridor(chunk.wseed, chunk.cell)
	...
	if chunk.style == WorldGen.BRUTAL_PASSAGE:
		...
		var fixture_y := chunk.ceil_h - 0.025
		var fi := 0
		for x in [3.7, 8.3]:
			for z in [3.7, 8.3]:
				var variant := posmod(
					int(chunk._r(2124 + fi) * 3.99) + fi, 4)
	...
	if dead:
		return
	var huge := chunk.style == WorldGen.BRUTAL_ATRIUM \
		or chunk.style == WorldGen.BRUTAL_SANCTUM \
		or chunk.style == WorldGen.BRUTAL_WATER_COURT
	var passage := chunk.style == WorldGen.BRUTAL_PASSAGE
	var light := chunk._make_main_light(flicker, lens,
...
func _brutalist_wall_detail(dir: int, plane: float) -> void:
	...
	var face := plane + n * chunk.T * 0.5
	for y in [1.18, 2.38, 3.58]:
		if y > chunk.ceil_h - 0.4:
			continue
...
func _column(p: Vector3, height: float, wide := 0.72) -> void:
	chunk._box(p + Vector3(0, height * 0.5, 0),
		Vector3(wide, height, wide), Mats.brutal_structure())
	chunk._box(p + Vector3(0, height - 0.32, 0),
...
func _upper_gallery(axis_x: bool, side: float, height: float) -> void:
	...
	chunk._box(p, slab, Mats.brutal_structure(), false)
	...
	chunk._box(edge, rail, Mats.brutal_steel(), false)
	for t in [-4.8, -3.2, -1.6, 0.0, 1.6, 3.2, 4.8]:
		var rp := edge + (Vector3(t, -0.36, 0) if axis_x else Vector3(0, -0.36, t))
		chunk._box(rp, Vector3(0.055, 0.78, 0.055), Mats.brutal_steel(), false)
...
func _black_court(cross_walk := true) -> void:
	if cross_walk:
		for x in [2.25, 9.75]:
			for z in [2.25, 9.75]:
				chunk._box(Vector3(x, 0.022, z), Vector3(3.25, 0.028, 3.25),
					Mats.brutal_black_water(), false)
	else:
		chunk._box(Vector3(6.0, 0.022, 6.0), Vector3(7.4, 0.028, 4.2),
			Mats.brutal_black_water(), false)
...
func _brutal_passage() -> void:
	...
	var lid_size := Vector3(chunk.S, 0.30,
	...
	for side in [-1.0, 1.0]:
		...
		var info: Dictionary = chunk._edge_info(chunk.cell, dir)
	...
	var tray_s := Vector3(chunk.S, 0.08, 0.34) if along_x \
		else Vector3(0.34, 0.08, chunk.S)
	...
	if chunk._r(2190) < 0.72:
		...
		if not solid_sides.is_empty():
			...
			_brutal_annex_door_at(p, chunk._wall_facing(int(chosen["dir"])),
				"tunnel")
...
func _tunnel_box(pos: Vector3, size: Vector3, part: String,
	...
	var mesh := chunk._box(pos, size, use_mat, collide)
...
func _brutal_tunnel_side(along_x: bool, side: float, _dir: int,
	...
	if bool(info["wall"]):
		...
		var solid_size := Vector3(chunk.S, TUNNEL_HEIGHT,
			TUNNEL_WALL_T) if along_x \
			else Vector3(TUNNEL_WALL_T, TUNNEL_HEIGHT, chunk.S)
	...
	var centre := clampf(float(info["t"]), width * 0.5 + 0.45,
		chunk.S - width * 0.5 - 0.45)
	var a := centre - width * 0.5
	var b := centre + width * 0.5
	for bounds in [[0.0, a], [b, chunk.S]]:
	...
	var header_h := TUNNEL_HEIGHT - chunk.BRUTAL_DOOR_TOP
	var header_p := Vector3(centre,
		chunk.BRUTAL_DOOR_TOP + header_h * 0.5, plane) if along_x \
		else Vector3(plane, chunk.BRUTAL_DOOR_TOP + header_h * 0.5, centre)
	...
	var outer := chunk.S - chunk.T if side > 0.0 else chunk.T
...
func _maybe_brutal_annex_door(salt: int, chance := 0.44) -> bool:
	...
	if chunk.room_n != 1 or chunk._r(salt) >= chance:
		return false
	var start := posmod(
		WorldGen.h(chunk.wseed, chunk.cell.x, chunk.cell.y, salt + 1), 4)
	for step in 4:
		var dir := (start + step) % 4
		if not chunk._solid_wall(dir):
			continue
		var along := 3.25 if chunk._r(salt + 2) < 0.5 else 8.75
		var p := chunk._wall_pt(dir, along, 0.13)
		return _brutal_annex_door_at(
			p, chunk._wall_facing(dir), "room_wall")
	return false
...
func _brutal_hall() -> void:
	...
	if chunk._r(2140) < 0.58:
		_upper_gallery(chunk._r(2141) < 0.5, -1.0 if chunk._r(2142) < 0.5 else 1.0,
			minf(chunk.ceil_h * 0.58, chunk.ceil_h - 2.5))


func _brutal_gallery() -> void:
	_perimeter_columns()
	_maybe_brutal_annex_door(2204, 0.50)
	var axis_x := chunk._r(2150) < 0.5
	_upper_gallery(axis_x, -1.0, minf(3.65, chunk.ceil_h - 2.2))
	if chunk.ceil_h > 8.0:
		_upper_gallery(axis_x, 1.0, minf(6.9, chunk.ceil_h - 2.2))
...
func _brutal_atrium() -> void:
	...
	var axis_x := chunk._r(2160) < 0.5
	_upper_gallery(axis_x, -1.0, minf(3.8, chunk.ceil_h - 2.4))
	_upper_gallery(not axis_x, 1.0, minf(7.1, chunk.ceil_h - 2.4))
...
func _brutal_ramp() -> void:
	...
	var along_x := chunk._r(2170) < 0.5
	var side := -1.0 if chunk._r(2171) < 0.5 else 1.0
	for i in 9:
		...
		chunk._box(p, s, Mats.brutal_structure())
	_maybe_brutal_annex_door(2216, 0.48)


func _brutal_service() -> void:
	var axis_x := chunk._r(2180) < 0.5
	for side in [-1.0, 1.0]:
		for y in [chunk.ceil_h - 1.1, chunk.ceil_h - 1.55]:
			var p := Vector3(6.0, y, 6.0 + side * 2.85) if axis_x \
				else Vector3(6.0 + side * 2.85, y, 6.0)
			var pipe := chunk._cyl(p, 0.11, 11.5, Mats.brutal_steel(), false)
...
func _brutal_sanctum() -> void:
	...
	for x in [1.35, 3.25, 8.75, 10.65]:
		_column(Vector3(x, 0, 6.0), chunk.ceil_h, 0.52)
	if chunk.ceil_h > 8.2:
