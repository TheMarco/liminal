...
func prison_cell_fixture_audit() -> Dictionary:
	...
	if chunk.theme != 8:
		return report
	var valid_style = chunk.style == WorldGen.PRISON_CELLBLOCK \
		or chunk.style == WorldGen.PRISON_CELLS
	for node in chunk.find_children("*", "Node3D", true, false):
		...
		if not bool(node.get_meta("prison_cell_context", false)) \
				or not valid_style:
			report["violations"] += 1
		var expected_path = chunk.PRISON_BUNK_PATH if prop == "double_bunk" \
			else chunk.PRISON_TOILET_PATH
